...
const TestResult = () => {
  ...
  const selectSuggestion = (title) => {
    ...
    setTimeout(() => {
      handleSearchWithTitle(title);
    }, 100);
  };

  // Separate search function that accepts title parameter
  const handleSearchWithTitle = async (title) => {
    const searchTitle = title || jobTitle;
    ...
    if (!isValidJobTitle(searchTitle)) {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/api/test/results/${encodeURIComponent(searchTitle)}`
      );
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                <input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === "Enter") {
                      handleSearchWithTitle();
                      setShowSuggestions(false);
                    }
                  }}
                  onFocus={() => {
                    if (jobTitle.trim().length > 0 && suggestions.length > 0) {
                      setShowSuggestions(true);
                    }
                  }}
                  className="w-full px-4 py-3 pl-12 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all duration-200 text-gray-800"
                />
                <svg
                ...
                {showSuggestions && suggestions.length > 0 && (
                  <div className="absolute z-50 w-full mt-2 bg-white border-2 border-violet-200 rounded-xl shadow-2xl max-h-64 overflow-y-auto">
                    ...
                    <div className="p-8">
                      ...
                      <div className="mb-6">
                        <div className="flex items-start justify-between mb-4">
                          ...
                          <span className={`px-4 py-2 rounded-full text-xs font-bold border shadow-sm ${isSubmitted ? 'bg-gradient-to-r from-emerald-100 to-green-100 text-emerald-700 border-emerald-200' :
